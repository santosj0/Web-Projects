...
if(param())
{
	...
	my ($new_height, $new_width, $new_diameter);

	# Generates lists for scaling
	my @hw			= ($height, $width);
	my @di			= ($diameter);

	# Scales to desired height and width
	if($height > 800 or $width > 800)
	{
		$new_height 	= scale($height, \@hw, 1, 800);
		$new_width	= scale($width, \@hw, 1, 800);
	}
	else {
		$new_height 	= $height;
		$new_width	= $width;
	...
	if($diameter > 800)
	{
		$new_diameter	= scale($diameter, \@di, 1, 800);
	}
	else
	{
